...
Examples:
  ...
  python launcher.py --evaluate     # Run evaluation on saved conversation data
    ...
    if args.clinical:
        # Run the clinical chatbot (use the updated module)
        print("Starting PsyRA Clinical Chatbot (with vectorstore)...")
        from upd_main import main
        main()
    elif args.baseline:
        # Run the baseline chatbot
        print("Starting Baseline Clinical Chatbot (without vectorstore)...")
        from run_comparison import run_baseline
        run_baseline()
    elif args.compare:
